    ...
    if (check(false,data_base)){
        ...
        for(int i=0; i<9; i++){
            for(int j=0; j<9; j++){
                ...
                if (data_base[f][g] == 0 && d_p[f][g].size() == 0) return false;
                ...
                while(naked_twins.size() > 0){
                        // bisa jadi satu nanti di satukan
...
bool solve_back_plus(int (&data_base)[9][9]){
    if (!solution_found){
        ...
        if (!solve_math(datum)) return false;
        list<int>::iterator ptr;
        for (ptr = d_p[row][col].begin(); ptr != d_p[row][col].end(); ++ptr){
            datum[row][col] = *ptr;
